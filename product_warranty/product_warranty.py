...
class ProductSupplierinfo(models.Model):
    ...
    def _get_default_instructions(self):
        ...
        return instr_obj.search([('is_default', '=', True)], limit=1)
    ...
    warranty_return_partner = fields.Selection(
        ...
        help="Who is in charge of the warranty return treatment toward"
             "the end customer. Company will use the current company delivery"
             "or default address and so on for supplier and brand"
             "manufacturer. Doesn't necessary mean that the warranty to be "
             "applied is the one of the return partner (ie: can be returned to"
             "the company and be under the brand warranty")
    return_instructions = fields.Many2one(
        comodel_name='return.instruction',
        string='Instructions',
        default=_get_default_instructions,
        help="Instructions for product return")
    active_supplier = fields.Boolean(
        string='Active supplier',
